...
var LightMode;
(function (LightMode) {
    LightMode["main_ligth_color"] = "#fff";
    LightMode["main_color"] = "#242b3d";
    LightMode["primary_color"] = "#596174";
})(LightMode || (LightMode = {}));
var DarkMode;
(function (DarkMode) {
    DarkMode["main_dark_color"] = "#242b3d";
    DarkMode["main_color"] = "#556ee6";
    DarkMode["primary_color"] = "#778BEB";
})(DarkMode || (DarkMode = {}));
//============ Mode Ui Colors ===============>
//============ Mode Controller ===============>
class Mode {
...
class Calc {
    ...
    Calcer() {
        this.inputs[2].addEventListener('input', (e) => {
            const ele = e.target;
            const num = parseInt(this.inputs[1].value);
            const price = +ele.value;
            if (price && num) {
        ...
        this.inputs[1].addEventListener('input', (e) => {
            const ele = e.target;
            const num = parseInt(this.inputs[2].value);
            const price = +ele.value;
            if (price && num) {
                this.inputs[3].value = `${(price * num).toString()}$`;
            }
...
class DataSter extends Calc {
    ...
    DrawUi() {
        if (this.array_of_data.length > 0) {
            ...
            this.array_of_data.forEach((ele, index) => {
                //======== Append Data ===============>
...
class DataEditer extends DataSter {
    ...
    Deleter(data) {
        ...
        data.forEach(ele => {
            ele.addEventListener('click', (e) => {
                ...
                //====== Filter Data =======>
                const currentData = data.filter((ele) => ele.id !== id);
    ...
    Filter_Data(data) {
        ...
        data.forEach(ele => {
            ele.addEventListener('click', (e) => {
                ...
                const ele_filter_data = ele_data.filter((ele) => ele.id == id_confirm);
    ...
    Editer(data) {
        ...
        const ele_data = JSON.parse(localStorage.getItem('data'));
        const ele_filter_data = ele_data.filter((ele) => ele.id != data[0].id);
...
class Search extends DataEditer {
    ...
    Searcher() {
        this.search.addEventListener('input', (e) => {
            const ele_data = document.querySelectorAll('#name');
            let ele_filter;
